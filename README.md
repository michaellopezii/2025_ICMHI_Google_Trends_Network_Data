...
The transformation works as follows. Given a distance correlation matrix $\mathbf{C} \in \mathbb{R}^{n \times n}$ and a threshold $\theta \in \text{{0.4, 0.5, 0.6, 0.8}}$, we define the adjacency matrix $\mathbf{A} \in \text{{0,1}}^{n \times n}$ as:
...
0 & \text{if } C_{ij} < \theta
\end{cases}
\quad \text{for } i,j \in \text{{1,2,\ldots,n}}$$
